...
const Requests = () => {
  ...
  const userRequests = mockRequests.filter(request => request.owner === user?.username);
  ...
  const getRequestTypeBadge = (type: RequestType) => {
    switch (type) {
      case 'maintenance':
        return <Badge variant="outline" className="border-orange-500 text-orange-700">Maintenance</Badge>;
      case 'cleaning':
        return <Badge variant="outline" className="border-purple-500 text-purple-700">Cleaning</Badge>;
      default:
        return null;
    }
  };
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        ...
        {userRequests.length > 0 ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="pt-2">
                      <p className="text-right">
                        <a href={`/requests/${request.id}`} className="text-nest-primary hover:text-nest-primary/80 text-sm">
                          View Details
                        </a>
                      </p>
                    </div>
          ...
          <div className="text-center py-12">
            <h3 className="text-xl font-medium text-gray-600">You don't have any service requests.</h3>
            <p className="mt-4">
              <a href="/new-request" className="text-nest-primary hover:text-nest-primary/80">

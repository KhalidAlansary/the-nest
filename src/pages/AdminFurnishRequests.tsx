...
const AdminFurnishRequests = () => {
  ...
  const fetchRequests = async () => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/furnish-requests`,
      );
  ...
  const updateStatus = async (
    _id: string,
    status: FurnishRequest["status"],
  ) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/furnish-requests/${_id}/status`,
        {
          method: "PATCH",
        ...
        <div className="container mx-auto max-w-7xl">
          <div className="mb-8">
            ...
            <p className="text-gray-600">
              Manage all incoming furnish service requests.
            </p>
          </div>
                          ...
                          {getFilteredRequests(tab).length > 0 ? (
                            getFilteredRequests(tab).map((r) => (
                              <TableRow key={r._id}>
                                ...
                                <TableCell>
                                  {r.propertyType} — {r.squareMeters}m²
                                </TableCell>
                                <TableCell>L.E.{r.budget}</TableCell>
                                <TableCell>
                                  {format(new Date(r.submittedAt), "PPP")}
                                </TableCell>
                                <TableCell>
